...
class SipHandler(val ctxt: Context) {
    ...
    private val configLoader = ImsConfigLoader(ctxt)
    private val mccmnc = mcc + mnc

    /* Carrier specific settings
     */
    val isControlSocketUdp = when(mccmnc) {
        "450006" -> true // LG U+ can only do UDP
        "208010" -> true // 20810 can do TCP and UDP. use this for testing
        else -> configLoader.isControlSocketUdp(mccmnc)
    }
    val forceSmsc = when(mccmnc) {
        "450006" -> "821080010585" // LG U+
        else -> configLoader.forceSmsc(mccmnc)
    }
    // Sess is more secure so default to it
    val requireNonsessAka = when(mccmnc) {
        "450006" -> true
        else -> configLoader.requireNonsessAka(mccmnc)
    }
    ...
    var onCancelledCall: ((handle: Object, from: String, extras: Map<String, String>) -> Unit)? =
        null
    var onOutgoingCallProgressing: (() -> Unit)? = null
    var onOutgoingCallStarted: (() -> Unit)? = null
    ...
    fun connect() {
        ...
        val pcscf = if (pcscfs.isNotEmpty()) {
            ...
            Rlog.w(TAG, "Had no Pcscf Sever defined, resolving via DNS")
            ...
            if (t != null) {
                ...
                Rlog.w(TAG, "Could not resolve P-CSCF via DNS, giving up")
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                ...
                Log.d(TAG, "Got exception in main/control socket", t)
            }
            Rlog.d(TAG, "Main control socket loop exited")
            socket.close()
        }
        CoroutineScope(Dispatchers.IO).launch {
            try {
                while (true) {
                    ...
                    val (reader, writer) = serverSocket.accept()
                ...
                Log.d(TAG, "Got exception in TCP server socket", t)
            }
        }
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                while (true) {
                    ...
                    serverSocketUdp.receive(dgramPacketIn)
                    ...
                    serverSocketUdp.send(dgramPacketOut)
                    writer.reset()
                }
            } catch(t: Throwable) {
                Log.d(TAG, "Got exception in UDP server socket", t)
            }
        }
    }
            ...
            while (true) {
                ...
                if (outBufIdx >= 0) {
                    ...
                    while(bufPos < outBufInfo.size) {
                        ...
                        val frameSize = AMR_FRAME_BYTES_WITH_HEADER.getOrNull(ft)
                        if (frameSize == null) {
                            Rlog.w(TAG, "Unsupported encoded AMR ft=$ft size=${outBufInfo.size} pos=$bufPos")
                            break
                        }
                        if (bufPos + frameSize > outBufInfo.size) {
                            Rlog.w(TAG, "Truncated encoded AMR frame ft=$ft frameSize=$frameSize size=${outBufInfo.size} pos=$bufPos")
                            break
                        }
...
a=sendrecv
    ...
    private fun extractCallerIdentity(request: SipRequest): Map<String, String> {
        val identityHeader = listOf(
            "p-asserted-identity",
            "remote-party-id",
            "from"
        ).firstNotNullOfOrNull { header -> request.headers[header]?.firstOrNull() } ?: ""

        val uri = Regex("<([^>]*)>").find(identityHeader)?.groups?.get(1)?.value
            ?: Regex("(sip:[^;>, ]+|tel:[^;>, ]+)").find(identityHeader)?.groups?.get(1)?.value
            ?: identityHeader
        val displayName = identityHeader.substringBefore("<")
            .trim()
            .trim('"')
        val number = uri
            .substringAfter("sip:", uri)
            .substringAfter("tel:", uri)
            .substringBefore("@")
            .substringBefore(";")
            .filter { it.isDigit() || it == '+' || it == '*' || it == '#' }

        return mapOf(
            "from" to number.ifEmpty { uri },
            "caller-uri" to uri,
            "caller-name" to displayName,
            "raw-from" to (request.headers["from"]?.firstOrNull() ?: ""),
            "raw-p-asserted-identity" to (request.headers["p-asserted-identity"]?.firstOrNull() ?: ""),
            "raw-remote-party-id" to (request.headers["remote-party-id"]?.firstOrNull() ?: "")
        )
    }
    ...
    fun handleCall(request: SipRequest): Int {
        ...
        onIncomingCall?.invoke(
            Object(),
            m,
            callerIdentity + mapOf("call-id" to request.headers["call-id"]!![0])
        )
...
""".toSipHeadersMap()
        ...
        when (sms.type) {
            SmsType.RP_DATA_FROM_NETWORK -> {
                ...
                try {
                    ...
                    Log.d(TAG, "Failed sending SMS to framework", t);
                }
            }
            SmsType.RP_ACK_FROM_NETWORK -> {
                try {
                    ...
                    Log.d(TAG, "Failed sending SMS ACK to framework", t)
                }
            }
    ...
    fun sendSms(
        ...
        val smsc =
            if (smsSmsc != null && decodableSmsc) smsSmsc
                ...
                try {
                    ...
                    try {
                        Log.d(TAG, "getSmscIdentity failed", t)
                        ...
                        Log.d(TAG, "smscAddress failed", t)
                        null
                    }
                }
            }
        ...
        val smscIdentityString = smscIdentity?.toString()
        val dest =
            if(smscIdentityString != null)
                if (smscIdentityString.startsWith("sip:") || smscIdentityString.startsWith("tel:"))
                    smscIdentityString
                else
                    "sip:$smscIdentityString"
            else if (smsc != null)
                "sip:+$smsc@$realm"
            else
                "sip:$realm"
        ...
        setResponseCallback(
            ...
            { resp: SipResponse ->
                Rlog.d(TAG, "SMS MESSAGE response ${resp.statusCode} ${resp.statusString}")
